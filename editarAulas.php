...
if($_GET["ID"]!=NULL and $_GET["ID"]!=""){
    $value=$_GET["ID"];
}else{
    header("location:listagemAulas.php");
}
...
<body class="bg-secondary">
    <div class="container bg-white rounded mt-4 mb-4 pt-2 pb-3 panel">
        <p class="h3 mt-4 text-center fw-bold text-title user-select-none">Editar Aula</p>
        ...
        <div class="container-sm mt-4 mb-4">
            <div class="col col-11 offset-1">
                ...
                <?php
                    ...
                    try{
                        $seleciona=mysqli_query($conexao, "SELECT * FROM aula WHERE ID=$value");
                ...
                <form action="backend/updateAula.php" method="POST">
                        ...
                        <div class="col col-12 col-sm-9 col-lg-8">
                            <div class="row">
                                ...
                                <div class="col col-2 col-sm-2 col-lg-1">
                                    ...
                                    <input class="btn-outline-green form-control w-100 text-muted d-none" type="hidden" value="<?=$campo["ID"]?>" name="ID" id="ID" required>
                                </div>
                                <div class="col col-10 col-sm-10 col-lg-11">
                                    <label class="mt-3 user-select-none" for="TITULO" ><span class="text-danger fw-bold">*</span>Título</label><br>
                                    <input class="btn-outline-green form-control w-100" value="<?=$campo["TITULO"]?>" type="text" name="TITULO" id="TITULO" maxlength="30" required autofocus>
                                </div>
                                <div class="col col-6">
                                    <label class="mt-3 user-select-none" for="DATA_AULA"><span class="text-danger fw-bold">*</span>Data da Aula</label><br>
                                    <input class="btn-outline-green form-control" value="<?=$campo["DATA_AULA"]?>" class="w-100" type="date" name="DATA_AULA" id="DATA_AULA" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col col-12 col-lg-12">
                                    <label class="mt-3 user-select-none" for="ID_PROFESSOR"><span class="text-danger fw-bold">*</span>Professor</label><br>

                                    <select class="form-select form-select-md btn-outline-green" id="ID_PROFESSOR" name="ID_PROFESSOR" required>
                                        <?php
                                            $seleciona=mysqli_query($conexao, "SELECT ID,NOME FROM professor ORDER BY NOME ASC");
                                            $nada=0;
                                            while($prof=mysqli_fetch_array($seleciona)){
                                                $nada=1;
                                                ?>
                                                    <option <?php if($campo["ID_PROFESSOR"] == $prof["ID"]){echo("selected");}?> value="<?=$prof["ID"]?>"><?=$prof["NOME"]?></option>
                                                <?php
                                            }
                                            if($nada==0){?>
                                                <option selected disabled value="0">Nenhum professor cadastrado</option>
                                                
                                                <script>
                                                    ip=document.getElementById('ID_PROFESSOR');
                                                    ip.classList.add('bg-secondary');
                                                    ip.classList.add('text-white');
                                                    ip.classList.add('fst-italic');
                                                    ip.classList.add('borderless');
                                                    ip.classList.remove('btn-outline-green');
                                                    ip.classList.add('btn-outline-secondary');
                                                    setTimeout(function(){
                                                        
                                                        loaded=false;
                                                        while(loaded==false){
                                                            try{
                                                                while(document.getElementById('submit').disabled!=false){
                                                                    loaded=false;
                                                                }
                            ...
                            <div class="row">
                                <div class="col col-12 col-lg-12">
                                    <label class="mt-3 user-select-none" for="DESCRICAO" ><span class="text-danger fw-bold">*</span>Descrição</label><br class="user-select-none">
                                    <textarea class="btn-outline-green form-control w-100" rows="8" type="text" name="DESCRICAO" id="DESCRICAO" maxlength="300" required><?=$campo["DESCRICAO"]?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="d-flex justify-content-center mt-4">
                        <input id="submit" class="btn-outline-green p-4 pt-1 pb-1 bg-button rounded btn text-white" value="Ok " type="submit">
                    </div>
                </form>
            </div>
        </div>
