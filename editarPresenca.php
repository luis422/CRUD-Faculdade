...
if($_GET["ID"]!=NULL and $_GET["ID"]!=""){
    ...
    header("location:listagemPresenca.php");
}
...
<body class="bg-secondary">
    <div class="container bg-white rounded mt-4 mb-4 pt-2 pb-3 panel">
        <p class="h3 mt-4 text-center fw-bold text-title user-select-none">Editar Presença</p>
        ...
        <div class="container-sm mt-4 mb-4">
            <div class="col col-11 offset-1">
                ...
                <?php
                    ...
                    try{
                        $id=$_GET["ID"];
                        $seleciona=mysqli_query($conexao, "SELECT * FROM presenca_aula WHERE ID=$id");
                ...
                <form action="backend/updatePresenca.php" method="POST">
                    <div class="row">
                        ...
                        <div class="col col-12 col-sm-9 col-lg-8">
                            <div class="row">
                                ...
                                <div class="col col-10 col-lg-11">
                                    <label class="mt-3 user-select-none" for="ID_AULA"><span class="text-danger fw-bold">*</span>Aula</label><br>

                                    <select class="form-select form-select-md btn-outline-green" id="ID_AULA" name="ID_AULA" required>
                                        <?php
                                            $seleciona=mysqli_query($conexao, "SELECT ID,TITULO,DATE_FORMAT(aula.DATA_AULA, '%d/%m/%Y') DATA_AULA FROM aula ORDER BY DATA_AULA DESC");
                                            $nada=0;
                                            while($aula=mysqli_fetch_array($seleciona)){
                                                $nada=1;
                                                ?>
                                                    <option name="opcAula" <?php if($campo["ID_AULA"] == $aula["ID"]){echo("selected");}?> value="<?=$aula["ID"]?>"><?=$aula["DATA_AULA"]," - ",$aula["TITULO"]?></option>
                                                <?php
                                            }
                                            if($nada==0){?>
                                                <option selected disabled value="0">Nenhuma aula cadastrada</option>
                                                
                                                <script>
                                                    ip=document.getElementById('ID_AULA');
                            ...
                            <div class="row">
                                <div class="col col-12 col-lg-12">
                                    <label class="mt-3 user-select-none" for="ID_ALUNO"><span class="text-danger fw-bold">*</span>Aluno</label><br>

                                    <select class="form-select form-select-md btn-outline-green" id="ID_ALUNO" name="ID_ALUNO" required>
                                        <?php
                                            $seleciona=mysqli_query($conexao, "SELECT ID,NOME FROM aluno ORDER BY NOME ASC");
                                            $nada=0;
                                            while($aluno=mysqli_fetch_array($seleciona)){
                                                $nada=1;
                                                ?>
                                                    <option name="opcAluno" <?php if($campo["ID_ALUNO"] == $aluno["ID"]){echo(" selected ");}?> value="<?=$aluno["ID"]?>" ><?=$aluno["NOME"]?></option>
                                                <?php
                                            }
                                            if($nada==0){?>
                                                <option selected disabled value="0">Nenhum aluno cadastrado</option>
                                                
                                                <script>
                                                    ip=document.getElementById('ID_ALUNO');
                                                    ip.classList.add('bg-secondary');
                                                    ip.classList.add('text-white');
                                                    ip.classList.add('fst-italic');
                                                    ip.classList.add('borderless');
                            ...
                            <div class="form-check m-1 mt-4">
                                <input class="form-check-input" onchange="refreshPresenca()" type="checkbox" <?php if($campo["PRESENTE"] == 1){echo("checked");}?> name="PRESENTE" id="PRESENTE">
                                <label class="form-check-label fw-bolder user-select-none" for="PRESENTE" id="LABEL_PRESENTE">
                                    Presente
                                </label>
                            </div>
                        </div>
